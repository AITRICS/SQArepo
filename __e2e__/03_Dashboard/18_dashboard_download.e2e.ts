...
const senarioName = '[18. 대시보드 다운로드 확인]';
...
test('대시보드 다운로드 사유 모달 확인', async ({ page }) => {
    ...
    const dimmedOverlay = page.locator('div.z-portal-background.bg-grey12');
    ...
    for (const reason of reasons) { // 다운로드 옵션 클릭
    ...
    const textbox = page.getByRole('textbox', { name: '~30자 이내로 입력해주세요.' }); // 직접입력 활성화 확인
    await expect(textbox).toBeVisible();
    await expect(textbox).toBeEnabled();
    await page.getByRole('textbox', { name: '~30자 이내로 입력해주세요.' }).fill('test test'); //텍스트 입력
    await page.waitForTimeout(500);
    await screenShot(page,senarioName,'직접입력 확인');
