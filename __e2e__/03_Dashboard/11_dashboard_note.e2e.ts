...
test('대시보드 노트 아이콘 확인', async({ page }) => {
  ...
  let noteCell = row.locator(`td:nth-child(${noteIndex})`); //첫번째 행의 노트 추출
  let noteButton = noteCell.locator('button');
  let btnText = await noteButton.innerText();

  if (btnText.trim() === '') {//노트 아이콘만 존재 
    ...
    await page.getByRole('button',{name:'icon-close'}).click(); //노트 닫기

    noteCell = row.locator(`td:nth-child(${noteIndex})`);
    noteButton = noteCell.locator('button');
    btnText = await noteButton.innerText(); //노트 아이콘에 노트 내용 확인
    ...
    await noteButton.click();
    await page.getByTestId('button-delete').click(); //노트 삭제
...
test('대시보드 노트 저장 버튼 확인', async({ page }) => {
  ...
  const latestNote = page.locator('xpath=/html/body/div[4]/div/div[3]/ul/li[1]'); // 또는 정확한 xpath 기반
  await expect(latestNote).toContainText('노트 저장 테스트');
  await page.waitForTimeout(1000);
...
test('대시보드 노트 삭제 확인', async({ page }) => {
  ...
  let currentCount = parseInt(noteCount.replace(/\D/g, ''), 10); //노트 카운트 확인
...
//  * 대시보드 노트 삭제 확인
//  */
// test('대시보드 노트 삭제 확인2', async({ page }) => {
//   let noteCount = await page.getByText(/노트 \d+/).textContent();
//   const currentCount = parseInt(noteCount.replace(/\D/g, ''), 10); //노트 카운트 확인
...
//  * 대시보드 노트 저장 확인
//  */
// test('대시보드 노트 저장 확인2', async({ page }) => {
//   let noteCount = await page.getByText(/노트 \d+/).textContent();
//   const currentCount = parseInt(noteCount.replace(/\D/g, ''), 10); //노트 카운트 확인
